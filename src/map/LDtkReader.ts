...
export class LdtkReader {
    ...
    CreateMap(levelName:string, tileset:string):LDtkMapPack {
        ...
        level.layerInstances.forEach(layer => {
            if(layer.__type === 'IntGrid') {
                var l = this.CreateIntGridLayer(layer, tileset, levelName)
                l.x = level.worldX;
                l.y = level.worldY;
                mappack.collideLayer = l;
                
            } else if (layer.__type === 'AutoLayer' || layer.__type === 'Tiles') {
                var l = this.CreateAutoLayer(layer, tileset,levelName);
                l.x = level.worldX;
                l.y = level.worldY;
    ...
    GetLevelFromID(id:number):Level {
         return this.ldtk.levels.find((l:Level) => l.iid == id);
    }
    GetLevelFromName(name:string):Level {
         return this.ldtk.levels.find((l:Level) => l.identifier == name);
    }

    CreateAutoLayer(layer:any, tileset:string, name:string = ""):Phaser.Tilemaps.TilemapLayer {
        ...
        let l = map.createLayer(0, ti, 0,0).setVisible(true).setName(name);
    ...
    CreateIntGridLayer(layer:LayerInstance, tileset:string, name:string = ''):Phaser.Tilemaps.TilemapLayer {
        let map:Phaser.Tilemaps.Tilemap;
        var csv = layer.intGridCsv;
        let arrayClone  = Object.assign([], csv);
        const newArr = [];
        ...
        csv = arrayClone;
        layer.intGridCsv = arrayClone;
        let ti = map.addTilesetImage(tileset);
        return map.createLayer(0, ti, 0,0).setVisible(false).setName(name);
    }
}
...
export class LDtkMapPack {
    ...
    GetTileAt(worldX:number, worldY:number):Phaser.Tilemaps.Tile {
        return this.collideLayer.getTileAtWorldXY(worldX, worldY);
    }
